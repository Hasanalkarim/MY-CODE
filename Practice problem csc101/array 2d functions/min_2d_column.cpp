...
void min_2d_column(float[10][10],int ,int,float[]); //find minimum value in 2d column and put it into y array
...
min_2d_column(x,row, column,y);
 ...
 //printing output
      ...
      cout<<endl<<"************** minium values are (column) *****************";
...
void min_2d_column(float x[10][10], int row,int column , float y[10]){
   ...
   for(i=0; i<column; i++){
        ...
        for(j=1; j<row; j++){   
            ...
            if(y[i]>x[j][i]){ 
                 ...
                 y[i]=x[j][i];   // new min value founnd seaving it on y array     
