...
void max_2d_row(float[10][10],int ,int,float[]); //find maximum value in 2d rowes and put it into y array
...
max_2d_row(x,row, column,y); //main funtion
 ...
 //printing output
      ...
      cout<<endl<<"************** maximum values in rowes *****************";
...
void max_2d_row(float x[10][10], int row,int column , float y[10]){
   ...
   for(i=0; i<row; i++){
       ...
       for(j=0; j<column; j++){
                ...
                if(y[i]<x[i][j]){ 
                 ...
                 y[i]=x[i][j];   // new max value founnd saving it on y array     
